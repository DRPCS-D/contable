 ...
 * necesitan la service_role key: crear una cuenta de Auth, borrarla y
 * cambiarle la contrasena.
 *
 * Todo lo demas (listar usuarios, editar nombre, activar/desactivar) lo hace
 * el cliente directo contra PostgREST, protegido por la RLS.
 */
...
interface Body {
  accion?: 'crear' | 'eliminar' | 'password'
  id?: string
  empresa_id?: string
  nombre?: string
  email?: string
...
const handler: ApiHandler = async (req, res) => {
  ...
  try {
    ...
    // El mensaje crudo puede filtrar detalles de configuracion del servidor
    console.error('api/admin/usuarios', e)
    return error(res, 500, 'Error interno del servidor.')
  }
}
